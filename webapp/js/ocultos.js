...
(async () => {
    ...
    try {
        ...
        console.error('⚠️ Firestore init failed in ocultos.js, using fallback:', e);
        db = getFirestore(app);
    }
    ...
    // Initialize Firestore locally to ensure valid instance
  ...
  `;
        ...
        try {
            ...
            querySnapshot.forEach((doc) => {
                if (doc.id !== currentUser.uid) {
                    ...
                    // 🛡️ SHOW ONLY HIDDEN USERS
                    // This page is specifically for viewing users you have hidden
                    if (!currentUserData.hiddenUsers || !currentUserData.hiddenUsers.includes(doc.id)) return;

                    // Still filter out blocked users (blocked is permanent/higher level)
                    ...
                    // Add mock location if not present
                    if (!data.location) {
                        const userId = doc.id;
        ...
        <div class="user-card glass rounded-2xl p-4 md:p-6 cursor-pointer relative group transition-all hover:bg-white/5" data-user-id="${user.id}">
          ...
          <div class="flex gap-2 items-center">
            ...
            <div class="flex gap-1.5">
                <!-- UNHIDE BUTTON -->
                <button class="unhide-user-btn bg-green-600 hover:bg-green-500 text-white w-full px-4 rounded-lg text-sm shadow-lg flex items-center justify-center transition-colors font-semibold" title="Restaurar usuario">
                  <i class="fas fa-eye mr-2"></i> Restaurar
                </button>
            </div>
          </div>
        </div>
    ...
    function attachCardListeners() {
        document.querySelectorAll('.user-card').forEach(card => {
            ...
            // Unhide User
            const unhideBtn = card.querySelector('.unhide-user-btn');
            if (unhideBtn) {
                unhideBtn.addEventListener('click', (e) => {
                    e.stopPropagation();
                    unhideUserAction(user);
                });
            }
        });
    }

    async function unhideUserAction(user) {
        if (!confirm(`¿Restaurar a ${user.alias || 'este usuario'}? Volverá a aparecer en las búsquedas.`)) return;

        try {
            const userRef = doc(db, 'users', currentUser.uid);
            await updateDoc(userRef, {
                hiddenUsers: arrayRemove(user.id)
            });

            // Update local state
            if (currentUserData.hiddenUsers) {
                currentUserData.hiddenUsers = currentUserData.hiddenUsers.filter(id => id !== user.id);
            }

            // Remove from view (since this view ONLY shows hidden users)
            removeUserFromView(user.id);
            showToast('Usuario restaurado correctamente', 'success');

        } catch (error) {
            console.error('Error unhiding user:', error);
            showToast('Error al restaurar usuario', 'error');
        }
    }
    ...
    // Placeholder for block action not used here
