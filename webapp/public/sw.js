...
const CACHE_NAME = 'tucitasegura-v3';
...
// Estrategia de fetch: Stale-While-Revalidate para dev, CacheFirst para assets inmutables
self.addEventListener('fetch', (event) => {
  ...
  // CACHE FIRST para assets inmutables (Vite hashed assets)
  // Utilizamos la ruta /assets/ generada por Vite
  const url = new URL(event.request.url);
  if (url.pathname.startsWith('/webapp/assets/') || url.pathname.startsWith('/assets/')) {
    event.respondWith(
  ...
  // NETWORK FIRST para todo lo demás (HTML, JSON, etc)
  event.respondWith(
